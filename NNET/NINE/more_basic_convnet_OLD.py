...
"""
I don't like the whole idea about padding the outside with zeros, 
in GO that really, really changes the meaning of what you're doing.
So, I should use padding valid. Also, it's way too big right now,
it's going to take forever to train. 
"""
...
W_conv2 = weight_variable([3,3,5,10])
b_conv2 = bias_variable([10])

h_conv2 = tf.nn.relu(conv2d(h_conv1, W_conv2) + b_conv2)


W_fc1 = tf.Variable(tf.random_uniform([4*4*10,1],-0.1,0.1))
b_fc1 = bias_variable([1])

h_conv2_flat = tf.reshape(h_conv2, [-1, 4*4*10])
...
class Very_Basic_ConvBot(GoBot):
  ...
  def get_best_move(self, board_matrix, previous_board, current_turn):
    ...
    best_value_move_pair = value_move_pairs[-1]
...
def train_convbot_on_randoms(load_path=None):
  b_c = Very_Basic_ConvBot(load_path=load_path)
  # for i in xrange(10):
  ...
  b_c.save_to_path('./saved_models/more_basic_convnet/trained_on_3_batch.ckpt')
...
if __name__ == '__main__':
  ...
  # train_convbot_on_randoms('./saved_models/more_basic_convnet/trained_on_2_batch.ckpt')
  # train_convbot_on_randoms('./saved_models/more_basic_convnet/trained_on_3_batch.ckpt')
  # train_convbot_on_randoms('./saved_models/more_basic_convnet/trained_on_1_batch.ckpt')
  print("trained!")
