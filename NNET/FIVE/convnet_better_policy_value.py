...
def softmax_np(np_matrix, temperature=1.0):
  shape = np_matrix.shape
  if len(shape) != 2:
    raise Exception('softmax_np not being called well.')
  flat = np_matrix.reshape(shape[0]*shape[1])
  e = np.exp(flat / temperature)
  dist = e / np.sum(e)
  to_return = dist.reshape(shape)
  return to_return
...
if __name__ == '__main__':
  ...
  # print(softmax_np(np.asarray([[1,2],[3,4]])))
  # print(softmax_np(np.asarray([[-1000,1],[1,1]])))
  # print(softmax_np(np.asarray([[-1000,1],[1,100]])))
  # exit()
